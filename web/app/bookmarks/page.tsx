...
import type { BookmarkResponse } from '~shared/schemas';
...
export default function BookmarksPage() {
  const [pinnedBookmarks, setPinnedBookmarks] = useState<BookmarkResponse[]>([]);
  const [recentBookmarks, setRecentBookmarks] = useState<BookmarkResponse[]>([]);
  const [recentAddedBookmarks, setRecentAddedBookmarks] = useState<BookmarkResponse[]>([]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {isLoading ? (
            ...
            pinnedBookmarks.map((bookmark) => (
              <BookmarkCard onUpdateBookmark={handleUpdateAction} key={bookmark._id} bookmark={bookmark} />
            ))
          )}
        </div>
      </section>

...
export function BookmarkCard({ bookmark, onUpdateBookmark }: BookmarkCardProps) {
  ...
  const handlePinBookmark = async () => {
    const { success } = await setBookmarkPinned({
      id: bookmark.id,
      isPinned:
        bookmark.is_pinned === BookMarkPinnedEnums.Pinned ? BookMarkPinnedEnums.UnPinned : BookMarkPinnedEnums.Pinned,
    });
    if (success) {
      handleUpdateBookmark();
    }
  ...
  const handleFavoriteBookmark = async () => {
    const { success } = await setBookmarkFavorite({
      id: bookmark.id,
      isFavorite:
        bookmark.is_favorite === BookMarkFavoriteEnums.Favorite
          ? BookMarkFavoriteEnums.UnFavorite
          : BookMarkFavoriteEnums.Favorite,
    });
    if (success) {
      handleUpdateBookmark();
    }

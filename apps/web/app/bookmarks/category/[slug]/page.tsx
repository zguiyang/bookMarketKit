...
import { BookmarkCategoryApi, BookmarkApi } from '@/api';
import { useState } from 'react';

export default function CategoryPage() {
  const params = useParams();
  const categoryId = params.slug as string;
  const [categoryInfo, setCategoryInfo] = useState<Partial<CategoryResponse>>({});
  const { onSuccess: onSuccessCategoryInfo } = useRequest(() => BookmarkCategoryApi.findOne(categoryId));
  const {
    data: bookmarkList = [],
    send: getPageList,
    loading: isLoading,
